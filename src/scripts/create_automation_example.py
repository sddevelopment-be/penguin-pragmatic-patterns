...
Creates the automation-example directory structure and copies/anonymizes files
"""
...
# Configuration
REPO_ROOT = Path("/home/runner/work/penguin-pragmatic-patterns/penguin-pragmatic-patterns")
TARGET_DIR = REPO_ROOT / "src" / "automation-example"

# Anonymization patterns
PATTERNS = [
    (r'sddevelopment-be', 'your-organization'),
    (r'SD Development', 'Generic Agent Framework'),
    (r'SDD Agent "([^"]+)"', r'Agent "\1"'),
    (r'SDD', 'Generic'),
    (r'Stijn Dejongh', 'the author'),
    (r'Stijn', 'the user'),
    (r'@stijn-dejongh', 'the author'),
    (r'stijn-dejongh', 'the-author'),
    (r'https://github\.com/sddevelopment-be/templates', 'https://github.com/your-organization/templates'),
    (r'sddevelopment-be\.github\.io', 'your-organization.github.io'),
    (r'patterns\.sddevelopment\.be', 'patterns.your-domain.com'),
    (r'http://sddevelopment\.be/', 'https://your-website.com/'),
]

def anonymize_content(content):
    """Apply all anonymization patterns to content"""
    for pattern, replacement in PATTERNS:
        content = re.sub(pattern, replacement, content, flags=re.IGNORECASE)
    return content

def copy_and_anonymize(source_path, target_path):
    """Copy a file and anonymize its content"""
...
def main():
    print("Creating automation-example directory structure...")
    ...
    print("Copying and anonymizing agent files...")
    
    # Copy agent files
    agent_dir = REPO_ROOT / ".github" / "agents"
    for agent_file in agent_dir.glob("*.agent.md"):
        copy_and_anonymize(agent_file, TARGET_DIR / "agents" / agent_file.name)
    
    # Copy specialist-agents.json
    json_file = agent_dir / "specialist-agents.json"
    if json_file.exists():
        copy_and_anonymize(json_file, TARGET_DIR / "agents" / "specialist-agents.json")
    
    print("Copying and anonymizing documentation files...")
    
    # Copy documentation files
    docs_dir = REPO_ROOT / "docs"
    doc_files = ["REPO_MAP.md", "SURFACES.md", "CONTEXT_LINKS.md", "WORKFLOWS.md"]
    for doc_file in doc_files:
        source = docs_dir / doc_file
        if source.exists():
            copy_and_anonymize(source, TARGET_DIR / "docs" / doc_file)
    
    print("Copying and anonymizing template files...")
    
    # Copy template file
    template_file = REPO_ROOT / "src" / "templates" / "TEMPLATE_SPECIALIST_AGENT.agent.md"
    if template_file.exists():
        copy_and_anonymize(template_file, TARGET_DIR / "templates" / "TEMPLATE_SPECIALIST_AGENT.agent.md")
    
    print("Creating README.md...")
    
    readme_content = """# Agent Framework Example
...
"""
    ...
    print("\n✅ Automation example created successfully!")
    print(f"\nLocation: {TARGET_DIR}")
    print("\nDirectory structure:")
    ...
    print("2. Verify all personal/company information has been removed")
    print("3. Commit the changes to the repository")
